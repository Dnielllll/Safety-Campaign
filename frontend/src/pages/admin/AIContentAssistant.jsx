...
export default function AIContentAssistant() {
  ...
  const [campaignType, setCampaignType] = useState("community");
  ...
  const detectCampaignType = (text) => {
    ...
    if (t.includes("fire") || t.includes("burn") || t.includes("safety")) return "safety";
    if (t.includes("health") || t.includes("disease") || t.includes("virus") || t.includes("dengue") || t.includes("medical")) return "health";
    if (t.includes("clean") || t.includes("garbage") || t.includes("environment") || t.includes("waste")) return "environment";
    if (t.includes("crime") || t.includes("security") || t.includes("theft") || t.includes("robbery") || t.includes("curfew")) return "community";
    if (t.includes("drug") || t.includes("substance") || t.includes("addiction")) return "safety";
    return "community"; // Default
  };

  // Update campaign type when prompt changes
  useEffect(() => {
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Campaign type will be automatically detected based on your prompt: {campaignType}
            </p>
          </CardContent>
          <CardFooter>
