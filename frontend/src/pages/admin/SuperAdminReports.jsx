...
import { BarChart3, Loader2, RefreshCw, Shield, Download } from "lucide-react";
...
export default function SuperAdminReports() {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      // Fetch all campaigns (super admin can see all campaigns)
  ...
  const handleConfirmedExport = async (password) => {
    try {
      if (pendingExportFormat === 'pdf') {
        ...
        // Add title
        doc.setFontSize(18);
        doc.text('System Campaign Engagement Report', 14, 20);
        doc.setFontSize(10);
        ...
        doc.save('system-campaign-engagement-report.pdf');
        ...
        link.download = 'system-campaign-engagement-report.csv';
        link.click();
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-display text-xl sm:text-2xl font-bold flex items-center gap-2">
            <BarChart3 className="h-5 w-5 sm:h-6 sm:w-6 text-primary" /> System Reports
          </h1>
          <p className="text-muted-foreground text-xs sm:text-sm">Performance of all campaigns across the entire system.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <Shield className="h-4 w-4 text-primary" /> System Overview — All Campaigns
          </CardTitle>
          <CardDescription>Total engagements per campaign across the entire system</CardDescription>
        </CardHeader>
      ...
      {data.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">System Campaign Engagement Details</CardTitle>
            <CardDescription>Detailed breakdown of engagement metrics across all system campaigns</CardDescription>
          </CardHeader>
          <CardContent>
